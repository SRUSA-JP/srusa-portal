...
- `src/`の公開関数には、関数名と一致する`<公開関数名>.test.js`を実装と同じディレクトリへ置きます。外部サービスのアダプターはデプロイ対象へテストを混ぜず、`tests/<アダプター名>/`へ実装構成を反映します。
- リポジトリ内モジュールのimportには、`package.json`で定義した`#src/`や`#functions/`を使用し、呼び出し元からの相対パスを使用しません。
